...
@section('content')
	...
	<div class="events-wrapper">
		...
		<div class="admin-body">
			<h3 class="table-title pull-left">{{ $events->total() }} Evenementen gevonden op '{{ $keyword }}'</h3>
			<div class="pull-right search search-admin">
				<form action="{{ url('/admin/evenementen/zoeken') }}" method="GET">
					<a href="{{ url('/admin/evenementen') }}"><i class="fa fa-times"></i></a><i class="fa fa-search search-button"></i><input value="{{ $keyword }}" placeholder="Zoeken" class="input-label-float search-active" type="text" name="search_event">
				</form>
			</div>
			<table class="table">
				<thead>
